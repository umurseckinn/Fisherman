...
import { GameEngine, CANVAS_WIDTH, CANVAS_HEIGHT, SEA_LEVEL_Y, ISLAND_CONFIG } from "@/game/GameEngine";
...
export default function Game() {
  ...
  const [timeLeft, setTimeLeft] = useState<number>(ISLAND_CONFIG[1].duration);
  const [currentIsland, setCurrentIsland] = useState(1);
  const [fuelCost, setFuelCost] = useState<number>(ISLAND_CONFIG[1].fuelCost);
  ...
  useEffect(() => {
    ...
    const islandConfig = ISLAND_CONFIG[currentIsland as keyof typeof ISLAND_CONFIG];
    const initialState: GameState = {
      score,
      island: currentIsland,
      fuelCost: islandConfig?.fuelCost ?? fuelCost,
      timeRemaining: islandConfig?.duration ?? 60,
      isPlaying: true,
      weather: "sunny",
      hook: {
        angle: Math.PI / 2,
    ...
    const engine = new GameEngine(ctx, initialState, {
      onGameOver: (finalScore, finalIsland, reason) => {
      ...
      onIslandComplete: (island) => {
        ...
        if (state) {
          ...
          setCurrentStorage(state.currentStorage || 0);
  ...
  const upgradeRod = () => {
    ...
    const cost = upgrades.rodLevel === 1 ? 60 : 120;
    if (score >= cost) {
      const newScore = score - cost;
      const newLevel = upgrades.rodLevel + 1;
      const newMaxAttempts = newLevel + 2;
      setScore(newScore);
  ...
  const upgradeBoat = () => {
    ...
    const cost = upgrades.boatLevel === 1 ? 100 : 200;
    if (score >= cost) {
      const newScore = score - cost;
      const newLevel = upgrades.boatLevel + 1;
      const newCapacity = newLevel === 2 ? 150 : 220;
  ...
  const handleNextLevel = () => {
    ...
    if (currentIsland >= 5) return;
    setCurrentIsland(prev => {
      const nextIsland = Math.min(prev + 1, 5);
      const config = ISLAND_CONFIG[nextIsland as keyof typeof ISLAND_CONFIG];
  ...
  const storageRatio = upgrades.storageCapacity > 0 ? (currentStorage + weightDisplayOffset) / upgrades.storageCapacity : 0;
  ...
  return (
    <div className="relative w-full h-screen bg-slate-900 flex items-center justify-center overflow-hidden">
      <div className="relative w-full h-full max-w-[450px] max-h-[800px] bg-sky-100 shadow-2xl overflow-hidden md:rounded-[32px] md:border-8 md:border-slate-800">
                ...
                <div className="bg-white/90 backdrop-blur-sm border-2 border-white rounded-xl p-2 shadow-md w-32 flex flex-col gap-1 transition-all">
                  <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                     ...
                     <span className={`${storageRatio >= 0.96 ? 'text-red-500 animate-pulse' : storageRatio >= 0.81 ? 'text-orange-500' : storageRatio >= 0.61 ? 'text-yellow-600' : 'text-slate-500'}`}>
                       {(currentStorage + weightDisplayOffset).toFixed(1)} / {upgrades.storageCapacity} kg
                     </span>
                  </div>
              ...
              <Card className="bg-slate-50 border-2">
                ...
                <CardContent className="p-3 flex flex-col gap-2">
                  <div className="flex flex-col gap-2">
                    ...
                    <Button onClick={() => repairHook(1, 20)} disabled={hookAttempts >= maxHookAttempts || score < 20} variant="default" className="flex flex-col h-auto p-2 gap-1 bg-amber-500 hover:bg-amber-600 text-white">
                      <Anchor className="w-4 h-4" />
                      <span className="text-[10px] leading-tight">Olta Tamiri +1 ($20)</span>
                      <span className="text-[8px] opacity-70">{hookAttempts}/{maxHookAttempts}</span>
                    </Button>
                    <Button onClick={() => repairHook(2, 35)} disabled={hookAttempts >= maxHookAttempts || score < 35} variant="default" className="flex flex-col h-auto p-2 gap-1 bg-amber-600 hover:bg-amber-700 text-white">
                      <Anchor className="w-4 h-4" />
                      <span className="text-[10px] leading-tight">Olta Tamiri +2 ($35)</span>
                      <span className="text-[8px] opacity-70">{hookAttempts}/{maxHookAttempts}</span>
                    </Button>
                    <Button onClick={repairHookFull} disabled={hookAttempts >= maxHookAttempts || score < 50} variant="default" className="flex flex-col h-auto p-2 gap-1 bg-amber-700 hover:bg-amber-800 text-white">
                      <Anchor className="w-4 h-4" />
                      <span className="text-[10px] leading-tight">Olta Tamiri Full ($50)</span>
                      <span className="text-[8px] opacity-70">{hookAttempts}/{maxHookAttempts}</span>
                    </Button>
                    <Button onClick={upgradeRod} disabled={upgrades.rodLevel >= 3 || score < (upgrades.rodLevel === 1 ? 60 : 120)} className="flex flex-col h-auto p-2 gap-1 bg-blue-500 hover:bg-blue-600 text-white">
                      <Zap className="w-4 h-4" />
                      <span className="text-[10px] leading-tight">Daha İyi Olta (${upgrades.rodLevel === 1 ? 60 : 120})</span>
                      <span className="text-[8px] opacity-70">Lv.{upgrades.rodLevel}</span>
                    </Button>
                    <Button onClick={upgradeBoat} disabled={upgrades.boatLevel >= 3 || score < (upgrades.boatLevel === 1 ? 100 : 200)} className="flex flex-col h-auto p-2 gap-1 bg-purple-500 hover:bg-purple-600 text-white">
                      <Anchor className="w-4 h-4" />
                      <span className="text-[10px] leading-tight">Büyük Tekne (${upgrades.boatLevel === 1 ? 100 : 200})</span>
        ...
        {gameState === "gameover" && <GameOverModal score={score} island={currentIsland} reason={gameOverReason ?? undefined} onRetry={() => window.location.reload()} />}
        {gameState === "win" && (
